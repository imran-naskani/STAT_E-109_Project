...
#View(data)
#View(hf_data)
dim(hf_data[!complete.cases(hf_data),])

hf_data_inc <- hf_data[complete.cases(hf_data),] #incomplete cases
hf_data <- hf_data[complete.cases(hf_data),] #Only complete cases
...
multi.model <- lm(MORT_30_HF ~ ., data = hf_train[,5:28])
summary(multi.model)
vif(multi.model)


multi.aic <- stepAIC(multi.model, tarce = F)

multi.model <- lm(MORT_30_HF ~ per_white_non_hisp + med_inc_15plus_12mo + COMP_HIP_KNEE + 
                  MORT_30_AMI + MORT_30_COPD + MORT_30_PN + MORT_30_STK +
                  PSI_14, data = hf_train[,5:28])
summary(multi.model)
vif(multi.model)

#View(corrdata) <- data.frame(cor(hf_data[,5:28]))
