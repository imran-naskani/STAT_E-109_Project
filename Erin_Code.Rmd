...
mod_mod <- lm(PSI_90~median_age+per_white_non_hisp+med_inc_15plus_12mo+
                per_below_poverty+per_college_grad_deg_25_plus,mod_train)
...
mod_modf <- lm(PSI_90~median_age+per_white_non_hisp+per_college_grad_deg_25_plus,mod_train)
...
```{r}
#Take a full model with all numeric terms add census division hospand overall factor

mod_mod2 <- lm(PSI_90~median_age+per_white_non_hisp+med_inc_15plus_12mo+
                per_below_poverty+per_college_grad_deg_25_plus+
                 census_division+hospital_ownership+hosp_overall_rating,mod_train)
...
#conduct a step AIC analysis to prioritize features in model

new <- stepAIC(mod_mod2)
...
#Use model selected by stepAIC function
mod_mod2_f <- lm(PSI_90~hosp_overall_rating+per_college_grad_deg_25_plus,mod_train)
...
```{r}
#Take a full model with all numeric terms add census division and overall factor

mod_mod3 <- lm(PSI_90~median_age+per_white_non_hisp+med_inc_15plus_12mo+
                per_below_poverty+per_college_grad_deg_25_plus+hospital_ownership+
                 census_division,mod_train)
...
new <- stepAIC(mod_mod3)

```
```{r}
#Final model

mod_mod3f <- lm(PSI_90~per_white_non_hisp+per_college_grad_deg_25_plus+
                  census_division,mod_train)
...
mod_mod3f$coefficients
coef(summary(mod_mod3f))[, "Pr(>|t|)"]
...
pn_data <- data %>%  
  select(4:5,7:17,23) %>% #Narrow to features to use in the model (plus one quality measure)
  filter(hosp_overall_rating != 'NA') %>% #remove NAs
...
copd_data <- data %>%  
  select(4:5,7:17,21) %>% #Narrow to features to use in the model (plus one quality measure)
  filter(hosp_overall_rating != 'NA') %>% #remove NAs
